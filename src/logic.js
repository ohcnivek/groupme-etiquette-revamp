...
async function fetchGroups(userAPIKey) {
  let response = await axios.get(
    "https://api.groupme.com/v3/groups?token=" + userAPIKey
  );
  return response;
}
...
async function getGroupMessageCountAndMessages(group, params, userAPIKey) {
  ...
  try {
    let group_response = await axios.get(
...
async function getUserMap(group, userAPIKey) {
  ...
  let response = await getGroupMessageCountAndMessages(
    group,
    params,
    userAPIKey
  );
  ...
  while (!ranOutOfMessages) {
    ...
    try {
      response = await getGroupMessageCountAndMessages(
        group,
        params,
        userAPIKey
      );
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      currStateOfSender["messages_sent"] =
        currStateOfSender["messages_sent"] + 1;

      let likes_received = likers.length;

      for (let j = 0; j < likers.length; j++) {
        let liker_id = likers[j];
        if (sender_id === liker_id) {
          // self like
          likes_received--;
      ...
      currStateOfSender["likes_received"] =
...
async function getRankings(group, userAPIKey) {
  ...
  for (let [key, value] of response.entries()) {
    if (value["messages_sent"] > 0) {
      const userToRatio = [
        ((value["likes_received"] * 1.0) / value["messages_sent"]) * 1.0,
        value["name"],
      ];
  ...
  //top 10
  let portion = Math.round(ranking.length * 0.15);
  let topRanks = [];
